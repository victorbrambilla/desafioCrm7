...
export function CustomFooterStatusComponent(props) {
    ...
    const deleteAllContact = (id) =>{
        if(window.confirm('Delete all contacts?')){
            const newIds = JSON.stringify(id).replace(/"/g, '').replace(']', '').replace('[', '');
    ...
    return (
      <Box sx={{ padding: '10px', display: 'flex', justifyContent:'space-between' }}>
        <div>
          ...
          <Button size='small' onClick={()=>{deleteAllContact(props.selectionModel)}} color='error'>Delete all</Button>
          }

          {props.selectionModel.length === 1 && 
          <Box sx={{display:'flex', alignItems:'center'}}>
            <ModalPage id={props.selectionModel}/>
            <Button size='small' onClick={()=>{deleteContact(props.selectionModel)}} color='error'>Delete</Button>
          </Box>
          }
        </div>
